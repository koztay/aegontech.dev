...
export default function MediaSelector({ onSelect, onClose }:{onSelect:(url:string)=>void,onClose?:()=>void}){
      ...
      <div className="bg-white rounded shadow max-w-4xl w-full max-h-[80vh] overflow-auto p-4">
        ...
        {loading && <div>Loading...</div>}
          ...
          <div className="grid grid-cols-4 gap-3">
            {items.map(it=> (
              <div key={it.id} className="border rounded p-2 cursor-pointer hover:shadow" onClick={()=>{onSelect(it.storage_path); onClose?.();}}>
