...
export function PortfolioCard({ item, onExternalLink }: PortfolioCardProps) {
        ...
        <div className="group relative bg-white dark:bg-slate-900 rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 border border-slate-200 dark:border-slate-800">
            ...
            >
                ...
                <div className="absolute top-3 left-3">
                    <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold ${item.type === 'mobile'
                        ? 'bg-cyan-500 text-white'
                        : 'bg-indigo-500 text-white'
                        }`}>
                        {item.type === 'mobile' ? <Smartphone className="w-3.5 h-3.5" /> : <Globe className="w-3.5 h-3.5" />}
            ...
            <div className="p-5">
                ...
                <div className="flex flex-wrap gap-2">
                    {item.links.website && (
                        <button
                            onClick={() => handleLinkClick(item.links.website!)}
                    ...
                    {item.links.appStore && (
                        <button
                            onClick={() => handleLinkClick(item.links.appStore!)}
                    ...
                    {item.links.playStore && (
                        <button
                            onClick={() => handleLinkClick(item.links.playStore!)}
